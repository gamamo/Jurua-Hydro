...
###################################################################### Run ordinations e prepare the table for the analysis #####
             ...
             # then run the NMDS and store the first 2 axis of each plant group in a separate list
              ...
              #plots test
              plot(listnmdsab[[1]][,4],listnmdsab[[1]][,3],cex=0.5+species25list[[1]]$Trich.tana)
              plot(scores(mds.ab)[,1],listnmds[[1]][,3])
              plot(listnmdsab[[1]][,4],envi$hand)
              ,cex=1+species25list[[1]]$Lomag.guia/5)
              plot(scores(mds.ab)[,1],scores(mds.ab)[,2] ,cex=0.5+envi$topo/10)
                plot(envi$hand,envi$topo,cex=0.1+species25list[[1]]$Serpo.adna)
              abline(lm(envi$topo~envi$hand))
              summary(lm(envi$topo~envi$hand))
              
              plot(listNMDSenviab[[1]]$topo,listNMDSenviab[[1]]$MDS1,cex= species25list[[1]]$Trich.sp1/10)
              ...
              unlistlistNMDSenviab <- dplyr::bind_rows(listNMDSenviab[[1]],listNMDSenviab,listNMDSenviab[[3]])
              unlistlistNMDSenvipa <- dplyr::bind_rows(listNMDSenvipa[[1]],listNMDSenvipa,listNMDSenvipa[[3]])
