...
setwd("C:/workspace_Gabriel_Moulatlet/Hidrologia do jurua/analyses v1/dados floristicos originais")
...
# in order to have a index value for each subtransect
      ...
      write.csv(moistureindextablezing,"moistureindextablezing.csv")  # save a csv file
...
write.csv(sp_data, "ferndataMI.csv")
...
widetableMIsp <- widetableMI[,-c(1:2)]
widetableMIsp[widetableMIsp>0] <- 2
widetableMI <- cbind(widetableMI[,c(1:2)], widetableMIsp)

relabu <- decostand(widetable[,-c(1:2)], method = "total",na.rm=T )
moistureindextable <- relabu*widetableMIsp
moistureindextable <- cbind(widetableMI[,1:2],moistureindextable)
MIferns <- cbind(widetableMI[,1:2], rowSums(moistureindextable[,-c(1:2)]))
  colnames(MIferns)[3] <- "MI"  

write.csv(moistureindextable,"moistureindextable.csv")
write.csv(MIferns, "MIferns_persubunit.csv")
