      ...
      solo  <- read.csv("soil_results_GMM_v3_forR.csv")
      ...
      # importing species data, run ordinations and plot against environmental variables ####
            ...
            #  graphic 3: total NMDS1 x HAND ####
              ...
              #############################
              #  teste
              a <- cbind(HANDsubsettotal[[3]][,],listnmdstotal[[3]][,])
              a[which(a$handnew>10),]
              
              summary(lm(a[which(a$handnew<10),"MDS1"]~a[which(a$handnew<10),"handnew"]))
              gra1 <- ggplot(a[which(a$handnew>10),], aes(x=handnew, y=MDS1))+
                geom_point()+
                geom_smooth(method="lm")
              gra1
              
              gra2 <- ggplot(a, aes(x=as.character(TrNumber), y=handnew))+
                geom_boxplot()
              gra2
              ...
              eq<-as.formula(y ~  yo + (a*x)/(1+b*x)) #Michaelis-Menten
              nlmod.1<-nls(eq, start=var,trace=TRUE)
