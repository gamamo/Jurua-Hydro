...
################################################################### graphics nmds x hand #####
              ...
              nmdsHAND<- ggplot(unlistlistNMDSenvipa,aes(hand,MDS1))+
              ...
              mfrow=c(1,1)
              tiff(filename="RESU_nmds_hand.tiff", height = 10*mfrow[1], width = 12*mfrow[1],units = "in",res = 300) # set the pdf file
              par(mfrow=mfrow, mar=c(0.2,0.2,0.2,0.2), oma=c(2,1,.5,0.5), mgp=c(1.7,0.6,0))
              nmdsHAND
              dev.off()
              
              mfrow=c(1,1)
              tiff(filename="RESU_nmds_soil.tiff", height = 10*mfrow[1], width = 12*mfrow[1],units = "in",res = 300) # set the pdf file
              par(mfrow=mfrow, mar=c(0.2,0.2,0.2,0.2), oma=c(2,1,.5,0.5), mgp=c(1.7,0.6,0))
              nmdsSoil
              dev.off()
...
####################################################### regression tress ####
              ...
              for (i in unique(listNMDSenvipa[[j]]$surface)){
                ...
                tree <- mvpart(MDS1+MDS2~log(soloslinearint)+log(1+hand)+decli,data=temp,plot.add = FALSE,cp=0.01,xv="min")
